...
func _init_queue(files: Array) -> void:
	for info in files:
		...
		var file = FileAccess.open(info[0], FileAccess.READ)
...
func _load_process() -> void:
	...
	while not _files.is_empty():
		...
		buffer = JSON.stringify(json.result)
...
func _http_process() -> void:
	...
	while not _queue.is_empty():
		...
		var json = JSON.parse_string(binaries.get_string_from_utf8())
